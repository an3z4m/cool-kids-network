...
class Cool_Kids_API {
	...
	public function check_permissions() {
		...
		return current_user_can( 'administrator' ) || current_user_can( 'network_maintainer' );
	}
	...
	public function update_user_role( WP_REST_Request $request ) {
		...
		if ( empty( $user ) ) {
			...
			return new WP_Error( 'user_not_found', 'User not found with the provided information.', array( 'status' => 404 ) );
		}
		...
		if ( ! $user->has_cap( $new_role ) ) {
			...
			return new WP_Error( 'role_update_failed', 'Failed to update user role.', array( 'status' => 500 ) );
		}
